...
//            }
            if (token != null && jwtUtil.validateToken(token)) {
                ...
                servletRequest.getSession().setAttribute("token", token);

                // Store session information in attributes for later use
                attributes.put("sessionId", sessionId);
                attributes.put("token", token);

                log.info("Handshake initiated for session: {}", sessionId);
                return super.beforeHandshake(request, response, wsHandler, attributes);
            }
        }
        return false;
    ...
    public void afterHandshake(ServerHttpRequest request, ServerHttpResponse response, WebSocketHandler wsHandler, Exception exception) {
        ...
        HttpServletRequest servletRequest = ((ServletServerHttpRequest) request).getServletRequest();
        String sessionId = servletRequest.getSession().getId();
//        Map<String, Object> attributes = request.getPrincipal()
        log.info("Handshake completed for session: {}", sessionId);
